...
export default function PosAdvanceBooking() {
  ...
  const buildBookingOrder = async (): Promise<BookingOrder | null> => {
    ...
    // Sync to Main if advance booking sync enabled
    try {
      const { syncBookingOrderOptional } = await import("@/features/sync/optional-sync");
      await syncBookingOrderOptional(order);
    } catch {}
    return order;
  };
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="advance">
        <TabsList>
          ...
          <TabsTrigger value="booking">Appointments & Booking</TabsTrigger>
        </TabsList>
